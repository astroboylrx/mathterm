...
```bash
sudo apt install ./MathTerm-0.12.0-arm64.deb
```

Use the `x64` file instead on an Intel/AMD machine. To run the AppImage without installing it:

```bash
chmod +x MathTerm-0.12.0-arm64.AppImage
./MathTerm-0.12.0-arm64.AppImage
```
...
To build a specific release instead of the current development branch, run `git checkout v0.12.0` after cloning.
